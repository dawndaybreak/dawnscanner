...
task :create do
  ...
  open("KnowledgeBase.md", "w") do |file|
    ...
    file.puts "\nThe knowledge base library for Dawn version #{Codesake::Dawn::VERSION} contains #{checks.count} security checks."
    file.puts "---"
    checks.each do |c|
...
task :calculate do
  ...
  built_gem_path = "pkg/codesake-dawn-#{Dawn::VERSION}.gem"
  checksum = Digest::SHA512.new.hexdigest(File.read(built_gem_path))
  checksum_path = "checksum/codesake-dawn-#{Dawn::VERSION}.gem.sha512"
